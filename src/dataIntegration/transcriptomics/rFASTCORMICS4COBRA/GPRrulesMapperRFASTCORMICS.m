function [res] = GPRrulesMapperRFASTCORMICS(exp, x)
% The GPRrulesMapperRFASTCORMICS function computes the GPR rules

% USAGE:
%
%[res] = GPRrulesMapperRFASTCORMICS(exp, x)
%INPUTS:
% exp:                 cell array with the expression to evaluate from the rules field
% x                    value to plug in 

%OUTPUTS:
% res                  the numeric corresponding to the result of the
%                      calculation
